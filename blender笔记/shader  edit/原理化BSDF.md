...
既然你已经对原理化 BSDF 的每一个滑块都有了深入物理本质的理解，我们现在就站在一个光子的视角，从它接触材质表面开始，绘制一张完整的**“能量闯关地图”**。

在 Blender 的计算逻辑中，能量遵循严格的**先后顺序（Priority）**，每一层都会按照比例截留、消耗或重组能量。
...
- **顺序**：**1**。
- **操作**：**截留（减法）**。
- **物理逻辑**：根据 `Weight` 和 `IOR`，光子首先被这层膜反射掉一部分（产生最外层清晰高光）。剩下的能量才能进入下方。
- **特殊操作**：`Tint`（色调）会在此处吸收（消耗）特定波长的光。

### ⚪ 第二关：边缘光泽 (Sheen) —— 绒毛过滤层

紧贴在涂层下方的微观散射层。

- **顺序**：**2**。
- **操作**：**截留（减法）**。
- **物理逻辑**：光线射入时，Sheen 会优先拦截一部分原本属于底层的光，并将其向后散射（形成边缘柔光）。
- **能量结果**：Sheen 反射的光越多，底层分到的光就越少。

### 🔵 第三关：薄膜 (Thin Film) —— 光谱重组器

它不抢夺能量总量，而是改变能量的“色彩身份”。

- **顺序**：**3** (作用于随后的高光计算)。
- **操作**：**重组/干涉（相位变化）**。
- **物理逻辑**：它让光线在极薄的膜层间发生**波动干涉**。
- **能量结果**：它几乎不降低亮度，只是把原本的高光“染色”成五彩斑斓的偏光。

### 🔴 第四关：高光 (Specular) —— 镜面反射层

决定有多少光能被物体表面瞬间弹走。

- **顺序**：**4**。
- **操作**：**截留（减法）**。
- **物理逻辑**：受 `Metallic`（金属度）和 `IOR Level` 控制。
  - **金属**：抢走几乎所有能量做有色反射。
  - **非金属**：只抢走约 4% 能量做白色反射，剩下的光进入“肉”里。

### 🟠 第五关：内部核心层 —— 终极分配站

到这里，剩下的光线已经进入了物体内部，面临最后的三选一分配。

| **参数**                | **顺序** | **操作类型**         | **物理逻辑**                                                 |
| ----------------------- | -------- | -------------------- | ------------------------------------------------------------ |
| **Transmission (透射)** | **5.a**  | **重定向（转向）**   | 光线不再反弹，而是受 IOR 引导直接穿过物体。                  |
| **Subsurface (次表面)** | **5.b**  | **漫游（延迟反弹）** | 光线在内部不断碰撞（步长由 `Scale*Radius` 决定）后弹出来。   |
| **Diffuse (漫反射)**    | **5.c**  | **消耗/反弹**        | 光线在表面浅层被吸收一部分颜色（Base Color），剩下的乱弹出来。 |

### 🟡 编外层：自发光 (Emission) —— 能量叠加

- **操作**：**加法（不消耗外部能量）**。
- **物理逻辑**：它不看太阳（光源）的脸色，自己产生光。它会强行覆盖在上述所有计算结果之上。
...
- **Coat** 是最高级的拦截者。
- **Metallic** 对 **Transmission**、**SSS** 和 **Diffuse** 具有一票否决权（比例压缩）。
- **Thin Film** 是唯一的“和平主义者”，只改变颜色不抢能量。

现在你已经拥有了这一整套物理账本，这是否彻底解释了为什么你开启了金属度后，液滴的通透感（Transmission）和肉感（SSS）会瞬间消失？

你想让我针对你目前的**美妆产品渲染**（如：红色磨砂瓶身 + 亮银泵头），按照这个排序给你一个具体的**参数调节避坑指南**吗？
